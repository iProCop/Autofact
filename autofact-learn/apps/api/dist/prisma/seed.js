...
async function main() {
    ...
    const vezelDefects = [
        {
            ...
            photoUrl: 'https://images.unsplash.com/photo-1492144534655-ae79c964c9d7?auto=format&fit=crop&w=800&q=80',
        },
        {
            id: 'd2',
            view: 'side',
    ...
    const reports = [
        {
            ...
            notes: 'Рекомендую покупку. Капот в перекрасе (аккуратно). Ходовая без люфтов. Салон ухожен.',
        },
        {
    ...
    for (let i = 0; i < reports.length; i++) {
        const r = reports[i];
        const s = r.scores;
        const overall = (s.tech + s.body + s.paint + s.interior + s.tires + s.electrics) / 6;
        await prisma.carReport.create({
            data: {
                expertId: experts[r.expert].id,
                title: r.title,
                summary: 'Осмотр по чек-листу АвтоФакт. Интерактивная карта дефектов и баллы 1–10.',
                expertNotes: r.notes,
                vin: r.vin,
                make: r.make,
                model: r.model,
